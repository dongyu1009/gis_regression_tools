...
# the following 2 functions are forked by https://github.com/mps9506/Mann-Kendall-Trend
...
if __name__ == '__main__':
	mask_filename = ".\\GIMMS\\mask.tif"
	filenames = [".\\GIMMS\\c_GIMMS_1981.tif", 
				 ".\\GIMMS\\c_GIMMS_1982.tif",
				 ".\\GIMMS\\c_GIMMS_1983.tif",
				 ".\\GIMMS\\c_GIMMS_1984.tif",
				 ".\\GIMMS\\c_GIMMS_1985.tif",
				 ".\\GIMMS\\c_GIMMS_1986.tif",
				 ".\\GIMMS\\c_GIMMS_1987.tif",
				 ".\\GIMMS\\c_GIMMS_1988.tif",
				 ".\\GIMMS\\c_GIMMS_1989.tif",
				 ".\\GIMMS\\c_GIMMS_1990.tif",
				 ".\\GIMMS\\c_GIMMS_1991.tif",
				 ".\\GIMMS\\c_GIMMS_1992.tif",
				 ".\\GIMMS\\c_GIMMS_1993.tif",
				 ".\\GIMMS\\c_GIMMS_1994.tif",
				 ".\\GIMMS\\c_GIMMS_1995.tif",
				 ".\\GIMMS\\c_GIMMS_1996.tif",
				 ".\\GIMMS\\c_GIMMS_1997.tif",
				 ".\\GIMMS\\c_GIMMS_1998.tif",
				 ".\\GIMMS\\c_GIMMS_1999.tif",
				 ".\\GIMMS\\c_GIMMS_2000.tif",
				 ".\\GIMMS\\c_GIMMS_2001.tif",
				 ".\\GIMMS\\c_GIMMS_2002.tif",
				 ".\\GIMMS\\c_GIMMS_2003.tif",
				 ".\\GIMMS\\c_GIMMS_2004.tif",
				 ".\\GIMMS\\c_GIMMS_2005.tif",
				 ".\\GIMMS\\c_GIMMS_2006.tif",
				 ".\\GIMMS\\c_GIMMS_2007.tif",
				 ".\\GIMMS\\c_GIMMS_2008.tif",
				 ".\\GIMMS\\c_GIMMS_2009.tif",
				 ".\\GIMMS\\c_GIMMS_2010.tif",
				 ".\\GIMMS\\c_GIMMS_2011.tif",
				 ".\\GIMMS\\c_GIMMS_2012.tif",
				 ".\\GIMMS\\c_GIMMS_2013.tif",
				 ".\\GIMMS\\c_GIMMS_2014.tif",
				 ".\\GIMMS\\c_GIMMS_2015.tif"]
	years = [[1981, 1982, 1983, 1984, 1985, 1986, 1987, 1988, 1989, 1990, 1991, 1992, 1993, 1994, 1995, 1996, 1997, 1998, 1999
	...
	res_trend = copy.deepcopy(im_mask).astype(np.int32)
	res_h = copy.deepcopy(im_mask).astype(np.int32)
	res_p = copy.deepcopy(im_mask).astype(np.float32)
	res_z = copy.deepcopy(im_mask).astype(np.float32)
	del ds_mask 
	...
	for i in range(im_height1):
		for j in range(im_width1):
			if im_mask[i][j] == 0:
				res[i][j] = nodatavalue
				res_trend[i][j] = nodatavalue
				res_h[i][j] = nodatavalue
				res_p[i][j] = nodatavalue
				res_z[i][j] = nodatavalue
				continue
			sequence = []
			for k in range(len(data)):
				b1 = data[k][i][j]
				sequence.append(b1)
			model_san.fit(years, sequence)
			res[i][j] = model_san.coef_[0]

			trend,h,p,z = mk_test(sequence)
			res_trend[i][j] = trend
			res_h[i][j] = h
			res_p[i][j] = p
			res_z[i][j] = z
			
	# EVI = 100.0*(2.5*(im_data2.astype(np.float32)- im_data1.astype(np.float32)))/(( im_data2.astype(np.float32)+6.0*im_data1.astype(np.float32)-7.5*im_data3.astype(np.float32)+1))
	# ndvi_ = 255.0*(ndvi-np.min(ndvi))/(np.max(ndvi)-np.min(ndvi)).astype(np.int16)

	#ds_out = driver.Create("LSWI+'00'+str(i).tif",im_width1,im_height1,1,gdal.GDT_Byte)
	...
	ds_out = gdal.GetDriverByName("GTiff").Create("trend.tif",im_width1,im_height1,1,gdal.GDT_Float32)
	ds_out.SetGeoTransform(im_geotrans1)
	ds_out.SetProjection(im_proj1)
	ds_out.GetRasterBand(1).WriteArray(res_trend.astype(np.int32))
	ds_out.GetRasterBand(1).SetNoDataValue(nodatavalue)
	del ds_out
	
	ds_out = gdal.GetDriverByName("GTiff").Create("h.tif",im_width1,im_height1,1,gdal.GDT_Float32)
	ds_out.SetGeoTransform(im_geotrans1)
	ds_out.SetProjection(im_proj1)
	ds_out.GetRasterBand(1).WriteArray(res_h.astype(np.int32))
	ds_out.GetRasterBand(1).SetNoDataValue(nodatavalue)
	del ds_out
	
	ds_out = gdal.GetDriverByName("GTiff").Create("p.tif",im_width1,im_height1,1,gdal.GDT_Float32)
	ds_out.SetGeoTransform(im_geotrans1)
	ds_out.SetProjection(im_proj1)
	ds_out.GetRasterBand(1).WriteArray(res_p.astype(np.float32))
	ds_out.GetRasterBand(1).SetNoDataValue(nodatavalue)
	del ds_out
	
	ds_out = gdal.GetDriverByName("GTiff").Create("z.tif",im_width1,im_height1,1,gdal.GDT_Float32)
	ds_out.SetGeoTransform(im_geotrans1)
	ds_out.SetProjection(im_proj1)
	ds_out.GetRasterBand(1).WriteArray(res_z.astype(np.float32))
	ds_out.GetRasterBand(1).SetNoDataValue(nodatavalue)
	del ds_out
